    ...
    <div className="space-y-6" data-testid="knowledge-base">
      ...
      {showBlogModal && (
        <Card className="border-2 border-purple-500 shadow-lg">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Blog Artikel Maken</CardTitle>
              <Button variant="ghost" size="icon" onClick={() => setShowBlogModal(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            <CardDescription>
              Geselecteerde documenten ({selectedDocuments.size}) worden gebruikt om een SEO-geoptimaliseerd blog artikel te genereren
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium mb-2 block">Blog Titel</label>
              <Input
                placeholder="Bijv: De beste orthomoleculaire strategie bij rugpijn"
                value={blogForm.title}
                onChange={(e) => setBlogForm({ ...blogForm, title: e.target.value })}
                data-testid="blog-title-input"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Wordt automatisch geoptimaliseerd met lokale keywords (fysio zeist, Orthomoleculair Praktijk Zeist)
              </p>
            </div>
            ...
            <div>
              ...
              >
                ...
                {uniqueCategories.filter(cat => cat !== "Blog Articles").sort().map(cat => (
                  <option key={cat} value={cat}>{cat}</option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Eigen Instructies (Optioneel)</label>
              <Textarea
                placeholder="Bijv: Focus op praktische tips voor senioren, voeg meer wetenschappelijke referenties toe, gebruik een vrolijke tone..."
                value={blogForm.customInstructions}
                onChange={(e) => setBlogForm({ ...blogForm, customInstructions: e.target.value })}
                rows={4}
                data-testid="blog-instructions-input"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Geef specifieke instructies voor hoe je het artikel wilt laten genereren
              </p>
            </div>
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <ul className="text-xs text-blue-700 space-y-1">
                <li>• Automatische meta titel en beschrijving generatie</li>
                <li>• Lokale SEO keywords: fysio zeist, Fysiopraktijk Zeist, Orthomoleculair Praktijk Zeist</li>
                <li>• Nederlandse blog structuur met H1/H2/H3 headers</li>
                <li>• FAQ sectie voor featured snippets</li>
                <li>• Internal linking suggesties</li>
              </ul>
            </div>
            ...
            <div className="flex gap-2">
              <Button 
                ...
                disabled={creatingBlog || !blogForm.title.trim()}
                className="bg-purple-600 hover:bg-purple-700 flex-1"
                data-testid="create-blog-submit-btn"
              >
                {creatingBlog ? "Genereren..." : "Blog Artikel Genereren"}
              </Button>
